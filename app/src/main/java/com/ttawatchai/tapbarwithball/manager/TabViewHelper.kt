...
open class TabViewHelper : View {
  ...
  private var textIcons = mutableListOf(
    "home",
    "menu",
    "branch",
    "me"
  ...
  override fun onDraw(canvas: Canvas?) {
    ...
    bitmapsIcons
      ...
      .forEach {
        ...
        val y =
          if (isSelectedIndex) (-(it.value.height / 4F) * tabAnimationPercentage) else ((height / 2.75) - (it.value.width / 2)).toFloat()
    ...
    textIcons
      ...
      .forEach {
        ...
        val x = (it.index * sectionWidth) + (sectionWidth/numberOfTabs )
        val paint = if (isSelectedIndex) selectTabText else unSelectTabText
        canvas?.drawText(it.value, x, y, paint)
      }
